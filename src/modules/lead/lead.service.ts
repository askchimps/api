...
export class LeadService {
    ...
    try {
      ...
      if (createLeadDto.email || createLeadDto.phone_number) {
        ...
        if (createLeadDto.email) {
          (duplicateCheck.OR as Prisma.LeadWhereInput[]).push({
            email: createLeadDto.email,
          });
        }
        if (createLeadDto.phone_number) {
          (duplicateCheck.OR as Prisma.LeadWhereInput[]).push({
            phone_number: createLeadDto.phone_number,
          });
        }
        ...
        if (existingLead) {
          throw new BadRequestException(
            'Lead with this email or phone number already exists',
          );
        }
      }
      ...
      if (createLeadDto.agent_slug) {
        ...
        if (!agent) {
          throw new NotFoundException(
            'Agent not found, inactive, or does not belong to the organization',
          );
        }
      }

      const { organisation_slug, agent_slug, ...leadDataWithoutSlug } =
        createLeadDto;
      const leadData = {
        ...leadDataWithoutSlug,
        organisation_id: organization.id,
        next_follow_up: createLeadDto.next_follow_up
          ? new Date(createLeadDto.next_follow_up)
          : null,
      };
  ...
  > {
    ...
    try {
      ...
      return ApiResponse.success(
        'Priority leads retrieved successfully',
        response,
      );
    } catch (error) {
      this.logger.error(
  ...
  private async findLeadByIdOrPhone(
    idOrPhone: string,
    organisationSlug?: string,
  ): Promise<Lead | null> {
    const whereCondition: Prisma.LeadWhereInput = {
  ...
  async updateByIdOrPhone(
    ...
    try {
      ...
      const existingLead = await this.findLeadByIdOrPhone(
        idOrPhone,
        organisationSlug,
      );
      ...
      const orgId: number | undefined = existingLead.organisation_id;
      ...
      if (updateLeadDto.email || updateLeadDto.phone_number) {
        ...
        if (updateLeadDto.phone_number) {
          (duplicateCheck.OR as Prisma.LeadWhereInput[]).push({
            phone_number: updateLeadDto.phone_number,
          });
        }
      ...
      if (follow_ups_operation && follow_ups_value) {
        ...
        if (follow_ups_operation === 'increment') {
          ...
          updateData.follow_ups = Math.max(
            0,
            currentFollowUps - follow_ups_value,
      ...
      const lead = await this.prisma.$transaction(async (prisma) => {
        ...
        if (updateLeadDto.agent_slug !== undefined) {
          if (agent) {
            ...
            updateLeadDto.agent_slug === '' ||
            updateLeadDto.agent_slug === null
          ) {
  ...
  async update(
    ...
    try {
      ...
      const updateData: any = {
        ...updateDataWithoutSlug,
        next_follow_up: updateLeadDto.next_follow_up
          ? new Date(updateLeadDto.next_follow_up)
          : undefined,
      };
  ...
  async markInProcessByIdOrPhone(
    idOrPhone: string,
    organisationSlug?: string,
  ): Promise<ApiResponse<Lead>> {
  ...
  async unmarkInProcessByIdOrPhone(
    idOrPhone: string,
    organisationSlug?: string,
  ): Promise<ApiResponse<Lead>> {
