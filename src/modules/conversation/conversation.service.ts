...
export class ConversationService {
    ...
    async create(createConversationDto: CreateConversationDto): Promise<ApiResponse<Conversation>> {
        ...
        try {
            ...
            if (!agent) {
                throw new NotFoundException('Agent not found, inactive, or does not belong to the organisation');
            }
            ...
            if (lead) {
                if (lead.length < 8) {
                    ...
                    (whereCondition.OR as Prisma.LeadWhereInput[]).push({ phone_number: lead });
                }
            }
            ...
            if (lead) {
                ...
                if (!leadData) {
                    throw new NotFoundException('Lead not found or does not belong to the organisation');
                }
            }
            ...
            if (existingConversation) {
                throw new BadRequestException('Conversation with this name already exists in the organisation');
            }
            ...
            return ApiResponse.success('Conversation created successfully', result, 201);
